...
if [ $stage -le 1 ]; then
  ...
  # steps/make_mfcc.sh --cmd "$train_cmd" --nj $nj data/${data_dir}_${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${train}_sp data/lang $gmm_dir $ali_dir || exit 1
fi
...
if [ $stage -le 4 ]; then
  ...
  # We'll use a seventh of the data, since whole training set is very large.
  mkdir -p exp/nnet3${nnet3_affix}/diag_ubm
  temp_data_root=exp/nnet3${nnet3_affix}/diag_ubm

  num_utts_total=$(wc -l <data/${train}_sp_hires/utt2spk)
  num_utts=$[$num_utts_total/7]
  utils/data/subset_data_dir.sh data/${train}_sp_hires \
     $num_utts ${temp_data_root}/${train}_sp_hires_subset

  echo "$0: computing a PCA transform from the hires data."
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      --splice-opts "--left-context=3 --right-context=3" \
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${train}_sp_hires_subset \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $nj \
    ...
    ${temp_data_root}/${train}_sp_hires_subset 512 \
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj $nj \
    --num-processes $num_processes data/${train}_sp_hires \
...
if [ $stage -le 6 ]; then
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train}_sp_hires ${ivectordir}/${train}_sp_hires_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${ivectordir}/${train}_sp_hires_max2 exp/nnet3${nnet3_affix}/extractor \
    $ivectordir || exit 1;
fi
